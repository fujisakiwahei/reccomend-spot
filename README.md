...
現在は仕様書のPhase 2にあたる、7地域・10件の検証済みサンプルを実装しています。200件版へ拡張する前に、データ形式、操作感、画像クレジット、地図クラスタリングを確認する段階です。
...
観光地データは `data/spots/` の7地域JSONへ置きます。`data/dataset-manifest.json` の `datasetStage` は現在 `sample` です。
